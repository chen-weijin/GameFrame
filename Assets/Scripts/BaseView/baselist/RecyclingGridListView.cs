...
public class RecyclingGridListView : MonoBehaviour
{
    [Tooltip("子节点物体")]
    public RecyclingGridListViewItem ChildObj;
    [Tooltip("行间隔")]
    public float RowPadding = 15f;
    [Tooltip("列间隔")]
    public float ColPadding = 15f;
    [Tooltip("事先预留的最小列表高度")]
    ...
    public void SetItemNum(int num,int cCount)
    {
        colCount = cCount;
        rowCount = ((int)Mathf.Ceil(num / cCount));
        if (itemCount != num)
        {
            itemCount = num;
            // 先禁用滚动变化
    ...
    public delegate void ItemDelegate(RecyclingGridListViewItem item, int rowIndex);
    ...
    /// 复用的item数组
    /// </summary>
    protected RecyclingGridListViewItem[] childItems;
    ...
    /// </summary>
    public virtual void Refresh(RecyclingGridListViewItem item)
    {
    ...
    /// </summary>
    /// <param name="row">行号</param>
    protected RecyclingGridListViewItem GetRowItem(int row)
    {
        if (childItems != null &&
    ...
    protected virtual bool CheckChildItems()
    {
        ...
        if (rebuild)
        {
            ...
            if (childItems == null)
                childItems = new RecyclingGridListViewItem[childCount];
    ...
    {
        return RowPadding + ChildObj.GetComponent<RectTransform>().rect.height;
    }
    ...
    protected virtual void UpdateChild(RecyclingGridListViewItem child, int rowIdx)
    {
        ...
        else
        {
            ...
            {
                Debug.Log("RecyclingGridListView is missing an ItemCallback, cannot function", this);
                return;
            }

            // 移动到正确的位置
            var childRect = ChildObj.GetComponent<RectTransform>().rect;
            Vector2 pivot = ChildObj.GetComponent<RectTransform>().pivot;
    ...
    {
        // 列表高度
        float height = ChildObj.GetComponent<RectTransform>().rect.height * rowCount + (rowCount - 1) * RowPadding;
        // 更新content的高度
